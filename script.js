...
function getSpeed() {
    return 105 - parseInt(speedControl.value); // Invert the range so that higher value means faster speed
}

// Event listener to update the displayed speed value
speedControl.addEventListener('input', () => {
    speedValue.textContent = speedControl.value;
});

// Modify the sleep function to use the current speed value
function sleep(ms) {
...
async function runRace() {
    ...
    // Calculate dynamic timeout based on speed
    const speed = getSpeed();
    const dynamicTimeout = Math.max(TIMEOUT_MS, (ROWS * COLS) * speed*5);

    try {
        const racePromise = Promise.all(algorithms.map(algo => visualize(algo)));
        
        const timeoutPromise = new Promise((_, reject) => 
            setTimeout(() => reject(new Error('Race timed out')), dynamicTimeout)
        );
